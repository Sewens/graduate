...
class SentimentAnalyzer(object):
    ...
    def load(self):
        ...
        for line in open(u'Dict\程度级别词语.csv').readlines()[1:]:
            item = line.rstrip('\n').split(',')
            self.level_dict[unicode(item[0],'utf-8')] = level_match[int(item[1])]
        #读取否定词
        self.deny_set = set([item.rstrip('\n') for item in open(u'Dict\否定词.csv').readlines()[1:]])
        #读取情感词表
        self.sen_dict = {}   #得到的二元组，首先是极性，然后是强度
        for line in open(u'Dict\正向词.csv').readlines():
            item = unicode(line.rstrip('\n'),'utf-8')
            self.sen_dict[item] = (1,1)
        for line in open(u'Dict\负向词.csv').readlines():
            item = unicode(line.rstrip('\n'),'utf-8')
            self.sen_dict[item] = (2,1)
        for line in open(u'Dict\情感词汇本体.csv').readlines()[1:]:
            item = line.rstrip('\n').split(',')
            self.sen_dict[unicode(item[0],'utf-8')] = (self.map_sentiment(item[4]),sen_match[int(item[5])])
