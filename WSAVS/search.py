...
def _search_rst_insert(dic,key_word):
    print dic
    ...
    try:
        ...
        print '%s Exception!' % _search_rst_insert.__name__
        print 'Exception:%s'% e
    #设置自增序号作为id
    try:
        for item in dic:
...
def _search_rst_fetch(name):
    ...
    try:
        ...
        print '%s Exception!' % _search_rst_fetch.__name__
        print 'Exception:%s' % e
        connect.close()
        return -1
